...
- [x] `src/app/robots.ts` e `src/app/sitemap.ts` não listam nem excluem
...
- [x] `src/app/manifest.ts` e `src/app/api/**` não revelam rotas internas além do
      necessário.
- [x] Nenhum source map (`*.map`) é servido publicamente em produção.
- [x] `src/app/api/admin/**`, `src/app/api/**` genericamente: confirmar que um
...
- `src/app/api/**` (para checar mensagens de erro)
