...
- [x] Confirmar que o `headers()` de `next.config.ts` (fonte `/(.*)`) chega
      ...
      `src/app/api/**`, e que Cloudflare/CDN, se existir na frente, não
      remove nenhum deles.
...
- Fluxo de checkout do Stripe (`src/app/api/stripe/**`, `src/app/api/billing/**`)
...
existir** desde então, emitida pelo `src/proxy.ts` em toda resposta que ele
...
| `Content-Security-Policy` | `src/proxy.ts`, toda resposta do proxy | ✅ (sem nonce, por decisão documentada) |
...
- **Checkout do Stripe:** é **redirect**, não embutido,
  ...
  `checkout.stripe.com` (`src/app/api/billing/checkout/route.ts:229` devolve
