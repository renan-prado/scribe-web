...
- [ ] Nenhum `sk_live_`, `sk_test_`, service-role key do Supabase, ou chave da
      OpenAI aparece hardcoded em `src/lib/`, `src/app/`, `src/`, ou em `src/scripts/`.
...
- [ ] Toda variável exposta ao cliente usa o prefixo `NEXT_PUBLIC_` de
      ...
      `src/lib/env/server.ts`) vaza para um componente `"use client"` ou para o
      bundle do navegador.
- [ ] A anon key do Supabase (pública por design) não está fazendo trabalho
      ...
      RLS, não a chave.
- [ ] `src/scripts/with-env.mjs` continua abortando se achar `.env`, `.env.local`,
      `.env.development[.local]` ou `.env.production[.local]`, e se achar
      `sk_live_` dentro de `.env.dev`.

## Áreas do repositório a inspecionar

- `.env.example`, `src/scripts/with-env.mjs`, `.gitignore`
- `src/lib/env/server.ts`, `src/lib/env/client.ts`
- Qualquer arquivo em `src/app/` marcado `"use client"` que importe de `src/lib/`
