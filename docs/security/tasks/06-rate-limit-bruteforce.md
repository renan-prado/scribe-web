...
- [x] `src/app/sign-in`, `src/app/sign-up`, e o fluxo de reset de senha usam a
...
- [x] Nenhum endpoint de autenticação alternativo (ex.: um `route.ts`
      próprio em `src/app/api/` que faça login/verificação por fora do
...
- `src/lib/supabase/require-auth.ts` e qualquer client de auth em `src/lib/`
- `src/lib/log` (verificar o que é logado nos fluxos de auth)
...
`src/app/sign-in` e `src/app/sign-up` renderizam só o `GoogleSignInButton`, que chama
...
`src/app/api/**` (busca por `signInWithPassword`, `grant_type`, `/auth/v1/token`:
...
`src/app/auth/callback` loga só `error.message` e a atribuição de parceiro
...
`src/proxy.ts` emite uma CSP com `connect-src` restrito: um XSS aqui vaza a sessão,
