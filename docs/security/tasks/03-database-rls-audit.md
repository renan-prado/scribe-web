...
novo de `src/lib/db/usage.ts`. Ver "Rodada 2026-09-05".
...
- [x] Policies de INSERT/UPDATE testadas explicitamente, não só SELECT.
      Em particular: colunas de saldo/moedas (`src/lib/db/coins.ts`) e de conta
      (`src/lib/db/account.ts`) só mudam via `service_role` ou via policy que
...
- [x] Storage buckets (áudio dos chunks, se armazenado) seguem a mesma
      ...
      **N/A:** `GET /storage/v1/bucket` com service-role devolve `[]` nos dois
      ...
      vive no IndexedDB do navegador (`src/lib/chunk-store.ts`) até virar texto.
...
- `src/lib/db/account.ts`, `src/lib/db/coins.ts`
- `supabase/AGENTS.md`
...
| `llm_usage_events` | `for insert with check (user_id = auth.uid())` | B: `POST /rest/v1/llm_usage_events {"user_id":"<o meu>","route":"transcribe","model":"gpt-5.1","prompt_tokens":99999999,"total_cost_usd":12345.67}` → **HTTP 201** | Contabilidade, não dado de usuário: a linha forjada entra em `/admin/costs`, os números que definem o preço da moeda e que conciliamos com a fatura da OpenAI, indistinguível de uma medição legítima | **MEDIUM** | ✅ migração **0039** derruba a policy + `src/lib/db/usage.ts` passa a escrever com service-role recebendo `userId` de quem chama (14 pontos de chamada) |
...
- **0039** ficou de fora de propósito: aplicada antes do deploy do
  `src/lib/db/usage.ts` novo, a produção para de gravar telemetria em silêncio (o
