...
| 03 | MEDIUM | Supabase de **produção**, migração `0039` | Depende do DEPLOY do `src/lib/db/usage.ts` novo. Aplicar antes derruba a telemetria de custo em silêncio. Depois do deploy: `npm run db:push:prod -- --yes --include-all` (fora de ordem, porque a 0040 entrou antes) |
...
| 05 | - | `src/app/api/format-paragraphs/route.ts` | Rota sem NENHUM chamador no cliente, mas viva e cara. Limite apertado para 20/hora como contenção; apagar ou ligar é decisão do usuário |
