...
- **Dois ambientes isolados** (dev/prod) com Supabase, Stripe e domínio
  próprios, `src/scripts/with-env.mjs` injeta as variáveis; nunca existe
...
- **RLS é a linha de defesa principal** para dados de usuário, não há uma
  camada de autorização própria além disso e de `src/lib/entitlements/`.
...
- **Áreas administrativas**: `src/app/admin` (UI) e `src/app/api/admin/*` (features,
