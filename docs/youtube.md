...
Sobrou provedor hospedado. É `src/features/session/server/youtube/supadata.ts`, atrás da interface de
`src/features/session/server/youtube/transcript.ts`, a indireção existe porque este é o pedaço do
...
**A exceção que confirma a regra:** `src/features/session/server/youtube/oembed.ts` chama o YouTube
...
`src/features/session/server/youtube/metadata.ts` faz uma chamada de `gpt-4o-mini` a temperatura 0 sobre
