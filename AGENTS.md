...
São seis arquivos (`AudioStudio.tsx`, `useAudioCapture.ts`,
`useRecordingPresence.ts`, `RecordingWorkbench.tsx`, `RecordingNotesDock.tsx`
e `recording-notes.ts`); a pasta `session` é
