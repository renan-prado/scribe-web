...
/**
 * A pílula acima do título do hero. Por padrão diz "Ouça, relembre e coloque
 * em prática."; para quem chegou por um link de indicação, ela vira
 * **"Indicado por Fulano"**, com a foto de quem indicou.
 *
 ...
 *
 * ## Os três estados, e por que existe um esqueleto
 *
 * A primeira versão mostrava a frase padrão e a trocava quando a resposta
 * chegava. Funcionava, e era ruim de ver: quem vinha indicado lia a frase de
 * efeito e a via sumir. Agora quem decide o estado inicial é o
 * `HeroEyebrowScript`, um script que roda ANTES DO PRIMEIRO PAINT e marca
 * `data-scriba-ref` no `<html>` quando existe a pista da indicação:
 *
 ...
 * | presente, mas sem indicação viva | esqueleto | a frase padrão |
 *
...
const DEFAULT_TAGLINE = "Ouça, relembre e coloque em prática.";

/**
 * Teto para o esqueleto virar a frase padrão sozinho.
 *
...
function hasReferralHint(): boolean {
  try {
    ...
    // Cookies bloqueados: não há pista, não há selo. A pílula padrão é um
...
export function HeroEyebrow() {
  ...
  useEffect(() => {
    ...
    fetch("/api/referral/active", { signal: controller.signal, cache: "no-store" })
      ...
      .catch((err: unknown) => {
        ...
        // "Indicado por Fulano". O mesmo vale fora do StrictMode, em toda
  ...
  return (
    <div className="inline-flex min-h-[32px] items-center gap-2 self-start rounded-[22px] border border-scriba-hairline bg-scriba-paper px-3.5 py-[7px] pl-[9px] shadow-[0_4px_12px_rgba(79,168,240,.1)]">
      {settled ? (
        referral ? (
          <>
        ...
        <>
          <span className="lp-eyebrow-idle">
            <Tagline />
          </span>
          <span className="lp-eyebrow-pending items-center gap-2" aria-hidden>
...
function Tagline() {
  return (
    <>
      <span className="h-[5px] w-2.5 flex-none rounded-[3px] bg-scriba-yellow" />
      <span className="text-[10.5px] font-semibold tracking-[.03em] text-scriba-ink-soft">
        {DEFAULT_TAGLINE}
      </span>
    </>
  );
