 ...
 *
 * **Os endereços ANTIGOS não estão aqui, e não devem estar.** Eles são
 * `redirects()` do `next.config.ts`, que rodam ANTES deste proxy: quem abre
 * `/recordings` já chega aqui como `/home`, e o `?next=` do login guarda o
...
export async function proxy(request: NextRequest) {
  ...
  if (user && pathname === "/") {
    return applyCsp(NextResponse.redirect(new URL("/home", request.nextUrl.origin)));
  }
