 ...
 *
 * **UMA roupa só**, o mesmo cartão do bloco `conclusion`: superfície de
 * sessão e a marca do Scriba na pastilha. As duas frases que a IA escreve
 * sobre o texto têm a mesma roupa, uma abrindo e a outra fechando a leitura, e
 * **as cores saem de tokens de sessão**, então dentro de `.tone-study` o mesmo
 * cartão nasce na família do estudo sem uma linha a mais.
 *
 * Havia uma segunda, `"rule"` — filete à esquerda e texto solto —, e ela era a
 * PADRÃO, herdada de quando o resumo se montava ao vivo durante a gravação.
 * Aquela tela não existe mais, então a variante ficou sem nenhum consumidor e
 * continuou sendo o que se ganhava por esquecer de passar a prop: foi assim
 * que a landing e o `/admin` passaram a mostrar um desenho que o app tinha
 * abandonado. Duas roupas com uma em uso é um jeito de errar, não uma opção.
 *
...
export function LeadIdea({ label, text, tourId }: Props) {
  if (!text) return null;
