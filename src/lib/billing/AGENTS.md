...
- **A MEDIÇÃO, porém, é do cliente.** Quem conta os minutos e chama
  ...
  `src/lib/coins/require-balance.ts` recusa quem está zerado nas rotas de LLM, e é
...
`src/lib/coins/pricing.ts` (client-safe, espelhado pela migração) governa só o
...
Os motivos de CRÉDITO em `GrantReason` (`src/lib/db/billing.ts`) incluem os três da
...
`/api/stripe` está na allowlist do `src/proxy.ts` porque o Stripe chega sem
