 ...
 *   `(shell)`: o avatar, o saldo, o menu da conta. Consultado UMA vez por
 ...
 *
 * `/subscribe` e `/subscribe/return` são o fluxo de pagamento, telas cheias que não têm
 * (nem devem ter) barra, e `/refer` traz o próprio voltar. Uma lista de
 * exceções num `if` de pathname apodrece na primeira rota nova; um grupo de
 * rotas não: quem está dentro de `(shell)/` tem barra, quem está fora não tem,
...
export function AppHeaderShell({ account }: { account: ReactNode }) {
  return (
    ...
    // de página. Escrita aqui, ela vale para as sete telas de `(shell)` de uma
