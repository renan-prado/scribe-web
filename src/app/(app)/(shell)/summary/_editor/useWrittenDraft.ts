...
/**
 * O estado do editor de `/summary/new`: uma cópia no aparelho, sempre, e o banco
 * atrás dela.
 *
 ...
 *
 * **A URL passa a ser `/summary/<id>/edit` assim que a folha abre**, por
...
export function useWrittenDraft(input: {
   ...
   * tempo de sobra para a primeira letra: quem abre `/summary/new` e começa a
  ...
  /**
   * A URL vira `/summary/<id>/edit` assim que a folha abre, sem navegar.
   *
  ...
  useEffect(() => {
    if (input.id) return;
    window.history.replaceState(null, "", `/summary/${draftId}/edit`);
  }, [input.id, draftId]);
