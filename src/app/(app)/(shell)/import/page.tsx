...
type PageProps = {
  searchParams: Promise<{
    ...
    /** Os nomes que estes dois tiveram enquanto as URLs falavam português.
     *  Continuam sendo lidos porque este endereço é COLADO por gente: um link
     *  com `?inicio=` mandado num grupo de WhatsApp em março não deixa de
     *  existir porque a rota trocou de idioma. O `next.config.ts` não resolve
     *  este caso — ele redireciona caminho, e estes são parâmetros. */
    inicio?: string;
    fim?: string;
  }>;
};
...
/**
 * `/import`, onde se cola o link de um vídeo.
 *
 ...
 * /import?text=Assista%20isso%20https://youtu.be/XXXXXXXXXXX   (compartilhamento)
...
export default async function ImportPage({ searchParams }: PageProps) {
  ...
  const { url, text, v } = sp;
  // Os nomes antigos entram só quando os novos não vieram, então um link que
  // traga os dois é decidido pelo que está escrito hoje.
  const startParam = sp.start ?? sp.inicio;
  const endParam = sp.end ?? sp.fim;
  ...
  const start = shared ? (startParam ? parseTimecode(startParam) : shared.startMs) : null;
  const end = shared && endParam ? parseTimecode(endParam) : null;
