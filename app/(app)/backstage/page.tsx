    ...
    <div className="flex flex-col gap-6">
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-semibold">{money(summaryAll.totals.totalCostUsd)}</div>
            <p className="mt-1 text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-semibold">{money(summary30d.totals.totalCostUsd)}</div>
            <p className="mt-1 text-xs text-muted-foreground">
              ...
              <ul className="divide-y divide-border">
                {summary30d.byUser.slice(0, 8).map((u) => (
                  <li key={u.userId} className="flex items-center justify-between py-2 text-sm">
                    ...
                    <span className="font-mono text-xs">{money(u.totalCostUsd)}</span>
                  </li>
                ))}
              </ul>
            )}
              ...
              <ul className="divide-y divide-border">
                {summary30d.byRoute.map((r) => (
                  <li key={r.route} className="flex items-center justify-between py-2 text-sm">
                    ...
                    <span className="font-mono text-xs">{money(r.totalCostUsd)}</span>
                  </li>
                ))}
              </ul>
            )}
